...
// constructs query
func (dht *coralNode) newQuery(k string, f queryFunc) *dhtQuery {
	return &dhtQuery{
		key:         k,
		dht:         dht,
		qfunc:       f,
...
func (r *dhtQueryRunner) queryPeer(proc process.Process, p peer.ID) {
	...
	if err != nil {
		...
		for _, next := range res.closerPeers {
			if next.ID == r.query.dht.id { // don't add self.
